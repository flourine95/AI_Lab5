...
public class Puzzle {
    ...
    public int computeH1(Node currentState) {
        ...
        for (int i = 0; i < MAX_ROW; i++) {
            for (int j = 0; j < MAX_COL; j++) {
                tileCurrent = currentState.getTile(i, j);
                tileGoal = goalState.getTile(i, j);
    ...
    public int computeH2(Node currentState) {
        ...
        for (int i = 0; i < MAX_ROW; i++) {
            for (int j = 0; j < MAX_COL; j++) {
                int[] curPos = currentState.getLocation(goalState.getTile(i, j));
                int[] goalPos = goalState.getLocation(goalState.getTile(i, j));
                result += PuzzleUtils.manhattanDistance(curPos, goalPos);
            }
        }
        return result;
    }
